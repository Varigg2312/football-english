const DB_FOLDER = './';
// LA CLAVE PRIVADA HA SIDO EXTIRPADA. JAMÁS VUELVAS A PONERLA AQUÍ.
...
function playSound(name) { 
    try { 
        ...
        sfx[name].play().catch(e => console.log("Audio silenciado por el navegador")); 
    } catch(e){} 
}
...
async function initLeague() {
    ...
    try {
        ...
        setupSearch(); 
    } catch (error) { console.error("Error al leer el catálogo:", error); }
}
...
async function loadMatch(filename) {
    ...
    } catch (error) { alert("Error al abrir el documento táctico."); }
}
...
async function sendMessage() {
    ...
    if (!isVip && usedMessages >= FREE_LIMIT) { 
        alert("🚨 Trial ended! Tu acceso gratuito ha expirado."); 
        return; 
    }
    ...
    const loadingDiv = addMessage('The Gaffer is thinking...', 'bot-msg');
    
    try {
        // Corrección de la URL: se apunta a la dirección real de Cloudflare Workers
        ...
        if (!response.ok) throw new Error("Aduana cerrada o inaccesible");
        
        const data = await response.json(); 
        loadingDiv.innerText = data.reply;
        
    } catch (error) { 
        loadingDiv.innerText = "❌ Instructor unavailable. The connection failed."; 
    }
}
