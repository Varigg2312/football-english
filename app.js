...
async function initLeague() {
    ...
    if (savedUser && usersDB[savedUser]) {
        loginUser(savedUser);
        landingPage.classList.add('hidden'); // Saltar portada si ya estás dentro
        appInterface.classList.remove('hidden');
        appInterface.style.display = 'flex';
    } else {
        loadGuestData();
...
function logoutUser() {
    ...
    // location.reload(); // Opcional: recargar para ver portada de nuevo
}
...
async function sendMessage() {
    ...
    const isVip = (ui.passwordInput.value === VIP_PASSWORD);
    if (!text) return;
    if (!isVip && usedMessages >= FREE_LIMIT) { alert("🚨 Trial ended! Enter VIP password."); return; }
...
function speak(text) {
    ...
    if (synth.speaking) synth.cancel(); // Romper atasco
